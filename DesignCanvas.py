...
class DesignCanvas(QWidget):
    ...
    def mouseInCanvas(self, pos):
        ...
        if(x >=0 and x <= self.voucher.getPixelWidth() and y >=0 and y <= self.voucher.getPixelHeight()):
            return True
        else:
            return False
    ...
    def paintEvent(self, ev):
        ...
        p.setBrush(brush)
        #画背景，如无背景则画一矩形
        if self.voucher.getBackground() == None:
            p.setBrush(self.drawingRectColor) #实心
            p.drawRect(self.startPoint.x(), self.startPoint.y(),  self.voucher.getPixelWidth(), self.voucher.getPixelHeight())
        else:
            imgdata=base64.b64decode(self.voucher.getBackground())  #图片数据以base64格式存放
            pixImg = QPixmap()
            pixImg.loadFromData(imgdata)    #生成QPixmap图片
            p.drawPixmap(self.startPoint.x(), self.startPoint.y(), self.voucher.getPixelWidth(), self.voucher.getPixelHeight(), pixImg)
        #画各个元素矩形
        p.setBrush(brush)
        for i in range(0, self.voucher.getElementNumbers()):
            ...
            p.setPen(pen)
            em = self.voucher.getElement(i)
            rect = QRect(self.startPoint.x()+em.getStartPoint().x(), self.startPoint.y()+em.getStartPoint().y(),  em.getWidth(), em.getHeight())
            p.drawRect(rect)   #画方框
            txt = em.getText()
    ...
    def deleteElement(self):
        if self.currElement < 0:
            return
        ret = QMessageBox.information(self,"Question","是否删除元素" + str(self.currElement+1), QMessageBox.Yes | QMessageBox.No )
