...
class Element:
    ...
    def xcontains(self, e):
        x = e.x() - self.startPoint.x()
        y = e.y() - self.startPoint.y()
        if(x >= self.width-2 and x <= self.width+2) and (y >= 0 and y <= self.height):
            return True
        else:
            return False
    #点在下边上
    def ycontains(self, e):
        x = e.x() - self.startPoint.x()
        y = e.y() - self.startPoint.y()
        if(x >= 0 and x <= self.width) and (y >= self.height-2 and y <= self.height+2):
            return True
        else:
            return False
...
class Voucher:
    ...
    #点在凭证矩形中    
    def contains(self, e):
        if (e.x() >= 0 and e.x() <= self.pixelWidth) and (e.y() >= 0 and e.y() <= self.pixelHeight):
            return True
        else:
            return False
    ...
    def save(self, dbpath):
        ...
        sql = "INSERT INTO voucher (vname,vtext,vwidth,vheight,vpixelwidth,vpixelheight) \
            values ('%s','%s',%d,%d,%d,%d)" %(self.name,self.text,int(self.width) , int(self.height),int(self.pixelWidth) , int(self.pixelHeight))  
        c.execute(sql)
        #字段信息
        ...
        for i in range(len(self.elements)):
            ...
            sql = "INSERT INTO elements (ename,epx,epy,ewidth,eheight,eval,etext,etype,elen,efont,etrim,eprefix,eprefixText,epostfix,epostfixText,emeancol,emeancolval,echeckval,ehaligh,evalign,elinewrap,eprintborder) \
                values ('%s',%d,%d,%d,%d,'%s','%s','%s',%d,'%s','%s','%s','%s','%s','%s','%s',%d,'%s','%s','%s','%s','%s')" %(em.name,int(em.getStartPoint().x()),int(em.getStartPoint().y()),int(em.width),int(em.height),em.val,em.text,em.type,int(em.len),em.font.toString(),\
                em.trim,em.prefix,em.prefixText,em.postfix,em.postfixText,em.meanCol,int(em.meanColVal), em.checkVal,em.hAlign,em.vAlign,em.lineWrap,em.printBorder)  
            c.execute(sql)
