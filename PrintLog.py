...
class PrintLog:
    ...
    def loadPrintLog(self, prdate, vtext, nbegin, nend):  #
        ...
        if prdate != '' and vtext == '':
            ...
            sql = "select * from prlog where prdate='%s' and vtext like '%s%%' limit %d,%d" % (
                prdate, vtext, nbegin, nend)
        else:
    ...
    def deletePrintLogByID(self, lid):
        conn = sqlite3.connect(self.logdb)
        c = conn.cursor()
        sql = "delete from prlog where lid=%d" % int(lid)
        c.execute(sql)
        conn.commit()
        conn.close()
    ...
    def loadEmDataByLid(self, lid):  #
        conn = sqlite3.connect(self.logdb)
        c = conn.cursor()
        sql = "select * from emdata where lid=%d" % int(lid)
