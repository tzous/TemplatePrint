...
class DesignWindow(QtWidgets.QMainWindow,Ui_DesignWindow):
    ...
    def clickedbtnNewModel(self):
        ...
        if (self.edited or self.designCanvas.edited) and self.voucher != None:
            ret = QtWidgets.QMessageBox.information(self,"Question","当前模板已更改，是否需要保存？", QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No )
    ...
    def clickedbtnLoadJpg(self):
        if self.voucher == None: #在已打开凭证模板的情况下才能操作
            return
        self.designCanvas.setMultiSelect(False) #取消多选状态
    ...
    def clickedbtnDelJpg(self):
        if self.voucher == None: #在已打开凭证模板的情况下才能操作
            return
        self.designCanvas.setMultiSelect(False) #取消多选状态
        ret = QtWidgets.QMessageBox.information(self,"Question","是否从模板中删除当前背景图？", QtWidgets.QMessageBox.Yes|QtWidgets.QMessageBox.No )
        if ret == QtWidgets.QMessageBox.Yes:
