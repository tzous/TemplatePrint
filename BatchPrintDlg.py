...
class BatchPrintDlg(QtWidgets.QDialog, Ui_BatchPrintDlg):
    ...
    def __init__(self):
        ...
        self.num = 0  # 模板字段数
        self.HeaderList = []  # 表头
        self.DataModel = None  # 表格数据Model
        self.printer = None  # 打印机
    ...
    def clickedbtnOpenFile(self):
        filename, _ = QtWidgets.QFileDialog.getOpenFileName(self, '请选择打印文件', '',
                                                            'Excel表格(*.xls);;Excel2010表格(*.xlsx);;wps表格(*.et)')
        if filename:
            ...
            if os.access(filename, os.F_OK):
                ...
                if sheet.ncols < self.num:  # 检查字段数
                    QtWidgets.QMessageBox.information(self, "error", "数据文件列数少于模板字段，请检查！",
